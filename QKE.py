...
class QuantumKeyExchange:
    ...
    def apply_pauli_operations(self, sequence, participant, start_pos=0):
        ...
        for i, state in enumerate(sequence):
            ...
            if isinstance(new_state, dict) and 'particle_ids' in new_state:
                # Keep the particle IDs unchanged
                new_state['particle_ids'] = state['particle_ids']
    ...
    def perform_qka(self, leaders):
        ...
        for leader in leaders:
            ...
            # Display sequences using particle IDs
    ...
    def _display_sequences(self, sequences, group_members):
        ...
        for p in group_members:
            ...
            for seq_type in sequences[p.id]:
                # Get the first group's particle IDs for display
                if sequences[p.id][seq_type]:
                    particle_ids = sequences[p.id][seq_type][0]['particle_ids']
                    # Format the sequence with all M particles in one line
    ...
    def distribute_key(self):
        ...
        rows = [
            ["Four-Particle GHZ State", f"{num_four_particle_groups} times",
             f"{num_four_particle_groups * 3} participants"],
            ["Three-Particle GHZ State", f"{1 if remaining_after_four == 2 else 0} times",
             f"{2 if remaining_after_four == 2 else 0} participants"],
            ["Bell State", f"{1 if remaining_after_four == 1 else 0} times",
             f"{1 if remaining_after_four == 1 else 0} participants"]
        ]
        self.print_table(headers, rows)
        ...
        rows = [
            ...
            ["Security Check Pass Rate",
             f"{sum(1 for s in security_status if s[1] == 'PASSED')}/{len(security_status)}"]
        ]
        self.print_table(headers, rows)
    ...
    def _perform_bell_state_qkd(self, leader, follower):
        ...
        for p in [leader, follower]:
            decoy_states[p.id] = {}
            positions, bases, states = self.insert_decoy_states(sequences[p.id]['B'], self.d)
            decoy_states[p.id]['B'] = {
                'positions': positions,
                'bases': bases,
                'states': states
